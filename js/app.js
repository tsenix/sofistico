(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new Swiper(".main__slider", {
    ...
    da.init();
    const languageButton = document.querySelector(".header__lang-btn");
    if (languageButton) {
        const languageBody = document.querySelector(".header__lang-body");
        const languageLinks = languageBody.querySelectorAll(".header__lang-link");
        languageButton.addEventListener("click", (() => {
            languageBody.classList.toggle("active");
        }));
        for (const languageLink of languageLinks) languageLink.addEventListener("click", (() => {
            const textLink = languageLink.textContent;
            const textButton = languageButton.textContent;
            languageButton.textContent = textLink;
            languageBody.classList.remove("active");
            languageLink.textContent = textButton;
        }));
        document.addEventListener("click", (e => {
            ...
            if (!its_languageBody && !its_languageButton && languageBody_is_active) languageBody.classList.remove("active");
        }));
    }
    function checkBreadcrumbsWidth() {
        const breadcrumbs = document.querySelector(".breadcrumbs");
        if (breadcrumbs) {
            const breadcrumbsContainer = breadcrumbs.querySelector(".container");
            const breadcrumbsList = breadcrumbs.querySelector(".breadcrumbs__list");
            if (breadcrumbsList.scrollWidth > breadcrumbsContainer.clientWidth - 28) breadcrumbsList.classList.add("full-width"); else breadcrumbsList.classList.remove("full-width");
        }
    }
    document.addEventListener("DOMContentLoaded", (() => checkBreadcrumbsWidth()));
    window.addEventListener("resize", (() => checkBreadcrumbsWidth()));
    const blogVideos = document.querySelectorAll(".blog__video");
    if (blogVideos.length) for (const blogVideo of blogVideos) blogVideo.addEventListener("click", (() => {
        blogVideo.classList.add("play");
        const iframe = blogVideo.querySelector("iframe");
